\documentclass[cn,11pt,chinese]{elegantbook}
	...
	\usepackage[os=win]{menukeys} %使用目录、菜单、按键
		...
		\newmenumacro{\winpath}[bslash]{pathswithfolder} %修改为winpath
...
\chapter{在Word中写作时用Zotero插入参考文献}\label{ch:insert}
		...
		\begin{enumerate}
			...
			则会弹出\menu{Zotero Preferences}对话框，
	...
	\chapter[利用坚果云同步全文]{利用坚果云同步全文\footnote{如果没有在不同电脑同步题录和全文的需要，本章可以略过}}\label{ch:syn}		
		...
		\begin{enumerate}
			...
			点击下一步，设置需要同步的文献贮存的地址，如\autoref{fig:ch3JianguoReg3}所示，
			然后关闭此窗口或点击\menu{我已知道如何使用}。
			当然也可以在坚果云的网站\url{https://www.jianguoyun.com/d/signup}上注册用户,
			然后在\autoref{fig:ch3JianguoRe}点击\menu{登录}即可。
			...
			即图中Base directory的路径，在系统托盘中的坚果云图标上右击，
			然后选择\menu{同步文件夹-管理}，对同步的文件夹进行重新设置，
			如\autoref{fig:ch3SynFolder}所示。
			
			\item 在弹出的对话框中，点击
			\menu{移动}，然后输入或点击\menu{浏览}找到全文附件所在的目录，值得注意的是，在用\menu{浏览}
			找到文件夹，
			坚果云会自动在末尾加上“我的坚果云”，注意删除。接着坚果云会提示，
			文件夹已经存在，点击\menu{合并}即可，
	...
	\section{ZotFile插件设置及将附件转移到坚果云同步盘}\label{sec:zotFilePre}
		...
		\begin{enumerate}
			...
			\end{figure}
			\item 在弹出的对话框中点击\menu{General Settings},
	...
	\section{设置ZotFile插件使其支持更多的文件类型}\label{sec:zotFile_more_filetype}
		...
		\quad \quad 在Zotero中依次点击\menu{Tools-ZotFile Preferences...}，
...
\begin{enumerate}
	...
	在这个页面中也可以点击\menu{Manage Profile}
	...
	\begin{figure}[htbp]
		...
		\end{minipage}	 
	\end{figure}
	\item 在打开的页面中，\menu{Send More Invitations}来发送邀请，
...
\begin{enumerate}
	...
	然后在框中输入群组的名称，点击\menu{Search Groups}，如\autoref{fig:ch4ZotSearchGroup2}所示。
...
%待补充
	...
	\menu{Edit-Preferences-Advanced-Files and Folders}，在Data Directory Location下面显示的路径即是
	...
	\section{Zotero 标签的使用} \label{sec:tag}
		...
		\href{https://zhuanlan.zhihu.com/p/275707703}{Zotero分类、标签和关联的使用。}}。
			...
			我们如果使用自己的标签功能最好把这个功能禁用，方法是在Zotero中依次点击\menu{Edit-Preference-General-Miscellaneous}，在界面的中间位置，在
...
\menu{Set Color}，如\autoref{ch5SetTagColor}所示。
	   ...
	   有权限时在导入CNKI题录时同时下载全文的方法\url{https://zhuanlan.zhihu.com/p/90638718}}。
		...
		\section{中文作者后为“等”，英文为“et al”的设置方法}\label{sec:etal_set}	
			...
			\begin{enumerate}
				...
				点击\menu{JM Chinese Std GB/T 7714-2005 (numeric, Chinese)}前面的下载链接，
	...
	\section{使用宏，使中文作者后为“等”，英文为“et al”}\label{sec:etal}
		...
		\section{文献题目中的大小写问题}\label{sec:case}
			...
			\begin{enumerate}
				\item 选中需要转换的文献，
				在Zotero中依次点击：\menu{Tools-Developer-Run Javascript}，
...
%待补充
%\href{https://zhuanlan.zhihu.com/p/338159167}
 %
